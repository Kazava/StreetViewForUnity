...
public class MenuBehavior : MonoBehaviour {
	...
	void Update () 
	{
		...
		for(int i=0; i<_buttonCount;i++)
		{
			...
			if(i != _closest) 
			{
				...
				if(i == _currentSelection) { _buttons[i].GetComponent<Renderer>().material.color = Color.Lerp(_buttons[i].GetComponent<Renderer>().material.color, _selectedColor, 0.25f); }
				else { _buttons[i].GetComponent<Renderer>().material.color = Color.Lerp(_buttons[i].GetComponent<Renderer>().material.color, _baseColor, 0.25f); }
			}
			...
			case MenuState.ACTIVATING:
				...
				break;
			case MenuState.ACTIVE:
				Vector2 buttonCenter = _uiCam.WorldToScreenPoint(_buttons[i].GetComponent<Renderer>().bounds.center);
		...
		if(_currentState == MenuState.ACTIVE)
		{
			...
			if(_closest != -1)
			{
				...
				else
				{
					...
					if(_closest == _currentSelection) { _buttons[_closest].GetComponent<Renderer>().material.color = Color.Lerp(_selectedColor, _highlightColor, highlightPercent); }
					else { _buttons[_closest].GetComponent<Renderer>().material.color = Color.Lerp(_baseColor, _highlightColor, highlightPercent); }
	...
	public void setOpacity(float opacity)
	{
		...
		if(_hasSubLabel)
		{
			current = _subLabel.GetComponent<Renderer>().material.color;
			_subLabel.GetComponent<Renderer>().material.color = new Color(current.r, current.g, current.b, opacity);
		}
	}
}
