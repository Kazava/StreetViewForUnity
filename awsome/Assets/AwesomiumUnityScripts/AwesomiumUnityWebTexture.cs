...
public class AwesomiumUnityWebTexture : MonoBehaviour 
{
	...
	//public Example0 example0;
	public HandControllerStreetView handControll;
	
	[SerializeField]
	private bool m_Transparent = false;

	public GameObject refObect;
	...
	void OnMouseOver()
	{
		...
		if (m_WebView != null && !m_WebView.IsLoading)
		{
			if (GetComponent<GUITexture>())
			{
				...
				//Debug.Log ("DesingedMousex=" + MouseX  );
			}
			else if (GetComponent<Collider>())
			{
				RaycastHit HitInfo;
				var leapPosition = handControll.SendPosition();
				leapPosition = Camera.main.WorldToScreenPoint(leapPosition);
				//leapPosition.z =-10;
				//Debug.Log("mouse"+Input.mousePosition);
				//Debug.Log("Leap"+leapPosition);

				if(this.GetComponent<Collider>().Raycast(Camera.main.ScreenPointToRay(leapPosition), out HitInfo, 10000.0f))
				{			
					//MapMouseButtons(0);
					m_WebView.InjectMouseDown(MapMouseButtons(0));
				...
				}else if(this.GetComponent<Collider>().Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out HitInfo, 10000.0f))
				{			
					Vector2 v = HitInfo.textureCoord;
					v.y = 1.0f - v.y;
					v.Scale(new Vector2((float)m_WebView.Width, (float)m_WebView.Height));
	...
	void Update(){
				//var result = m_WebView.ExecuteJavaScript("document.title");
		/*		//Debug.Log( result.ToString() );
		var pinchChecker =handControll.SendPinch();
				if (pinchChecker == 1) {
						m_WebView.InjectMouseWheel (0, 3 * Time.deltaTime);
				} else if (pinchChecker == -1) {
						m_WebView.InjectMouseWheel (0, -3 * Time.deltaTime);
				} */
		}
}
